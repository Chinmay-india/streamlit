...
def check_query_string(app: Page, entries: dict[str, list[str]]):
    def _has_query_string_matching_entries():
        ...
        if not (url_query_string.keys() == entries.keys()):
            return False
        if not all(
            set(url_query_string.get(key)) == set(entries[key]) for key in entries
        ):
            return False
        return True
...
def test_default_page(app: Page) -> None:
    """Test that the home page of the hello app is displayed correctly."""
    check_query_params_printout(app, "Query params on page 1: {}")
...
def test_navigation_via_page_link(app: Page) -> None:
    """Test that navigating by page link induces the correct query params change."""
    # first pagelink goes to page 2
    navigate_to_page_via_page_link(app, 0)

    check_query_params_printout(
        app, "Query params on page 2: {'navigation': 'from page link on page 1'}"
    )
...
def test_navigation_via_page_link_with_embed_params(app: Page) -> None:
    ...
    check_query_params_printout(
        app,
        "Query params on page 2: {'navigation': 'from page link on page 1', 'embed': 'true', 'embed_options': 'option1'}",
    )
    check_query_string(
        app,
        {
...
def test_navigation_to_same_page_via_sidebar(app: Page) -> None:
    ...
    # first pagelink goes to page 2, to add query params to the URL
    navigate_to_page_via_page_link(app, 0)
    # simulate re-clicking on the page 2 sidebar nav, this should _clear_ the query params
    navigate_to_page(app, 1)

    check_query_params_printout(app, "Query params on page 2: {}")
    check_query_string(app, {})
...
def test_navigation_via_switch_page(app: Page) -> None:
    ...
    # first button goes to page 2
    navigate_to_page_via_button(app, 0)

    check_query_params_printout(
        app, "Query params on page 2: {'navigation': 'from switch page on page 1'}"
    )
...
def test_navigation_via_switch_page_with_embed_params(app: Page) -> None:
    ...
    check_query_params_printout(
        app,
        "Query params on page 2: {'navigation': 'from switch page on page 1'}",
    )
    check_query_string(
        app,
        {
...
def test_navigation_via_switch_page_callback(app: Page) -> None:
    """Test that navigating via switch_page as a callback induces the correct query params change."""
    # Page 2 has the button with switch_page as a callback
    navigate_to_page(app, 1)
    ...
    check_query_params_printout(
        app,
        "Query params on page 1: {'navigation': 'from switch page callback on page 2'}",
    )
    check_query_string(app, {"navigation": ["from switch page callback on page 2"]})


def test_navigation_via_switch_page_callback_with_embed_params(app: Page) -> None:
    """Test that navigating via switch_page as a callback induces the correct query params change."""
    # Page 2 has the button with switch_page as a callback
    navigate_to_page(app, 1)
    # start with embed options
    ...
    check_query_string(
        app,
        {
            "navigation": ["from switch page callback on page 2"],
            "embed": ["true"],
            "embed_options": ["option1"],
        },
    )
